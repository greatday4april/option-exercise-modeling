...
class Model:
    ...
    def get_income_tax(self, amount):
        ...
        return self.get_fica_tax(amount) + self.get_federal_income_tax(amount) + self.get_tax(self.STATE_TAX_BRACKETS, amount)

    def get_federal_income_tax(self, amount):
        return self.get_tax(self.INCOME_TAX_BRACKETS, amount)
    ...
    def get_tax(self, tax_brackets: List[TaxRate], amount: float):
        ...
        for idx in range(len(tax_brackets)):
            ...
            upper = tax_brackets[idx + 1].threshold if idx < len(
                tax_brackets) - 1 else float('inf')
    ...
    def compute(self, iso_exercise_units, nso_exercise_units):
        ...
        amt_tax_total = self.get_tax(
            ...
            (nso_exercise_units + iso_exercise_units) * spread
        )
        # when comparing with amt tax, dont include FICA tax
        amt_tax_due = max(0, amt_tax_total - self.get_federal_income_tax(
            self.taxable_income + nso_exercise_units * spread))
        ...
        cost_now = (iso_exercise_units + nso_exercise_units) * \
            self.strike_price + tax_due_now

        long_term_profit = (iso_exercise_units + nso_exercise_units) * \
            self.sell_price - cost_now - tax_after
        ...
        amt_tax_due_for_exercise_after_public = max(
            amt_tax_for_exercise_after_public - self.get_federal_income_tax(self.taxable_income + nso_exercise_units * new_spread), 0)
        ...
        original_tax_rate = tax_for_exercise_after_public / \
            ((self.sell_price - self.strike_price) *
             (iso_exercise_units + nso_exercise_units)) if iso_exercise_units + nso_exercise_units > 0 else (0.49)

        current_tax_rate = (tax_after + tax_due_now) / \
            ((self.sell_price - self.strike_price) *
             (iso_exercise_units + nso_exercise_units)) if iso_exercise_units + nso_exercise_units > 0 else (0.24)

        # sellable stock
        nso_units_first_vest = (
            self.nso_total_units + self.iso_total_units) * self.sell_month / 48 - self.iso_total_units
        ...
        # if i sell immediatly my ISO, it will be short-term and same as federal income tax
        sellable_stock_value_after_tax = sellable_stock_value - \
            self.get_income_tax(
                (sellable_nso + sellable_iso) *
                (self.sell_price - self.strike_price) + self.taxable_income
            ) + self.get_income_tax(self.taxable_income)

        sellable_nso_stock_value_after_tax = sellable_nso * (self.sell_price - self.strike_price) - self.get_income_tax(
            sellable_nso * (self.sell_price -
                            self.strike_price) + self.taxable_income
        ) + self.get_income_tax(self.taxable_income)

        return {
            "cost_now": '${}'.format(int(cost_now)),
            "total_tax_savings": '${}'.format(int(tax_savings)),
            "amt_tax_saving_vs_exercise_after_public": '${}'.format(int(amt_tax_due_for_exercise_after_public)),
            "long_term_profit_after_tax": '${}'.format(int(long_term_profit)),
            "sellable_stock_value_after_tax": '${}'.format(int(sellable_stock_value_after_tax)),
            'sellable_NSO_value_after_tax': '${}'.format(int(sellable_nso_stock_value_after_tax)),
            "orginal_tax_rate": '{}%'.format(int(original_tax_rate * 100)),
            "current_tax_rate": '{}%'.format(int(current_tax_rate * 100))
        }
...
if __name__ == '__main__':
    ...
    model.sell_month = st.sidebar.slider(
        'Months after grant to sell', 12, 24, 15, 1,
    )

    iso_exercise_units = st.sidebar.slider(
        'Number of ISO units to exercise', 0, model.iso_total_units, 100, 10,
    )

    nso_exercise_units = st.sidebar.slider(
        'Number of NSO units to exercise', 0, model.nso_total_units, 100, 10,
    )
    ...
    st.markdown(
        '### This assumes all exercised options are sold after holding period required by long term capital gain')
